(() => {
	const ed = O => {
		...
		h.p(`#_wqEr { padding: 10px; height: 24rem; overflow-y: scroll; outline: none; border: 1px solid #999; box-shadow: 2px 2px 2px #666 }\n`);
		h.p(`.ed_ful { position: absolute; top: 0; left: 0; background: snow }\n`);
		h.p(`.ed_btn { display:inline-block; margin: 0 1px; padding: 1px; vertical-align: middle }\n`);
		h.p(`.sept { width: 2px; height: 20px; background: url(../img/is.png) -400px 0 }\n`);
		h.p(`.icon { width: 20px; height: 20px; border: 1px solid transparent; background-repeat: no-repeat; background:url(../img/is.png) }\n`);
	...
	ed.sour = () => {
		let o = document.getElementById("_wqEx"),
			x = ed.g.clientWidth - 50, y = ed.g.clientHeight - 40, i = ed.g.innerHTML;
		if (o) {
			i = o.value;
			ed.g.innerHTML = i;
			ed.g.focus();
	...
	ed.full = () => {
		if (ed.o.className) {
			ed.o.className = "";
			ed.o.style.width = "";
			ed.g.style.height = "24rem";
		} else {
			ed.o.className = "ed_ful";
			ed.o.style.width = "calc(100vw - 20px)";
			ed.g.style.height = "calc(100vh - 100px)";
		}
	};
